...
def _build_bullet_summary_doc(
    ...
    if working_rows.empty:
        ...
        for _, row in working_rows.iterrows():
            ...
            # Explicit blank line between bullet entries for readability.
...
def _render_state_filter():
    ...
    return state_label, STATE_NAME_TO_CODE.get(state_label), state_note_slot
...
state_label, state_code, state_note_slot = _render_state_filter()
base_note_message = (
    "Select a state to view the current session's last action date."
    if not state_code
    else "Select archives to view the current session's last action date."
)
...
archive_payload_lookup: Dict[str, bytes] = {}
...
for uploaded_zip in uploaded_zips or []:
    ...
    try:
        ...
        archive_payload_lookup[uploaded_zip.name] = payload_bytes
...
for archive_path in selected_local_archives:
    ...
    try:
        ...
        zip_payloads.append(payload_bytes)
        archive_payload_lookup[archive_path.name] = payload_bytes
...
status_groups: Dict[str, List[str]] = (
    _group_archives_by_status(processed_archive_names)
    if processed_archive_names
    else {}
)

state_note_message = base_note_message
if state_code and zip_payloads:
    current_archive_names = status_groups.get("current", [])
    current_payloads = [
        archive_payload_lookup[name]
        for name in current_archive_names
        if name in archive_payload_lookup
    ]
    latest_current_action = (
        _collect_latest_action_date(current_payloads) if current_payloads else None
    )
    if latest_current_action:
        formatted_date = latest_current_action.strftime("%B %d, %Y")
        state_note_message = (
            f"{state_label} current session last action "
            f"(most recent in-session record): {formatted_date}."
        )
    elif current_archive_names:
        state_note_message = (
            "Current session archives detected, but no bill action dates were found."
        )
    else:
        fallback_latest_action = _collect_latest_action_date(zip_payloads)
        if fallback_latest_action:
            formatted_date = fallback_latest_action.strftime("%B %d, %Y")
            state_note_message = (
                "No current session archives detected; latest bill action "
                f"across selected files: {formatted_date}."
            )
        else:
            state_note_message = "No bill action dates were found in the selected archives."
